;var op_asset_settings = (function($){
    return {
        ...
        attributes: {
            ...
            step_2: {
                ...
                icon: {
                    ...
                    asset: ['core', 'video_player_viewport_fullscreen'],
                    showOn: {
        ...
        customInsert: function(attrs) {
            ...
            str = '[video_player_viewport_fullscreen' + tag_attrs + ' icon="' + icon + '"]' + content + '[/video_player_viewport_fullscreen]';
        ...
        customSettings: function(attrs,steps) {
            ...
            if (type == 'url') {
                attrs.content ? $('#op_assets_core_video_player_viewport_fullscreen_url').val(op_base64decode(attrs.content) || '') : '';
                attrs.url1 ? $('#op_assets_core_video_player_viewport_fullscreen_url1').val(op_base64decode(attrs.url1) || '') : '';
                attrs.url2 ? $('#op_assets_core_video_player_viewport_fullscreen_url2').val(op_base64decode(attrs.url2) || '') : '';
            } else if (type == 'youtube') {
                attrs.content ? $('#op_assets_core_video_player_viewport_fullscreen_youtube_url').val(op_base64decode(attrs.content) || '') : '';
            } else {
                attrs.content ? $('#op_assets_core_video_player_viewport_fullscreen_content').val(op_base64decode($(attrs.content).attr('alt')) || '') : '';
            }

            //set choosen style
            OP_AB.set_selector_value('op_assets_core_video_player_viewport_fullscreen_style_container', style);

            //Set choosen icon
            var icon = $('#op_assets_core_video_player_viewport_fullscreen_icon_container');
            OP_AB.set_selector_value(icon.attr('id'),(attrs.icon || ''));
           
            //Set uploaded icon
            OP_AB.set_uploader_value('op_assets_core_video_player_viewport_fullscreen_upload_icon', attrs.upload_icon);
            ...
            $.each(attrs,function(i,v){
                if (i == 'auto_buffer') {
                    $('#op_assets_core_video_player_viewport_fullscreen_' + i).attr('checked',(v=='Y')).trigger('change');
                } else {
                    $('#op_assets_core_video_player_viewport_fullscreen_' + i).val(v).trigger('change');
                }
            });
        }
    };
