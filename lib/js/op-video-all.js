...
(function ($) {
	...
	mejs.MediaElementPlayer.prototype = {
		...
		setControlsSize: function() {
			...
			// outer area
			// volume control falls into second row (chrome, ff), that's why we reduce the rail by 1.
			rail.width(railWidth - 1);
			// dark space
	...
	if (typeof opjq != 'undefined') {
		opjq.fn.mediaelementplayer = function (options) {
			if (options === false) {
				this.each(function () {
					var player = opjq(this).data('mediaelementplayer');
					if (player) {
						player.remove();
					}
					opjq(this).removeData('mediaelementplayer');
				});
			}
			else {
				this.each(function () {
					opjq(this).data('mediaelementplayer', new mejs.MediaElementPlayer(this, options));
				});
			}
			return this;
		};
...
opjq(document).ready(function($){
    ...
    var mediaElementSelector = '.video-plugin, .video-plugin-new, .audio-plugin, .video-fullscreen';
    ...
    $('.op-popup-content').each(function () {
        ...
        $(this).find(mediaElementSelector).each(function () {
            ...
            /**
             * Remove the iframe source to ensure it doesn't load
             * (and start playing video), and detach the
             * iframe form dom to ensure iframe didn't
             * already start loading (because it
             * doesn't take into account
             * removing of the src once
            ...
            $iframeDetached = $iframe.detach();
            $iframeDetached.appendTo($iframeParent);
        });
    });
