...
/**
 * OP_AWeberServiceProvider
 *
 * Provides specific AWeber information or implementing OAuth.
 * @uses OP_OAuthServiceProvider
 * @package
 * @version $id$
 */
class OP_AWeberServiceProvider implements OP_OAuthServiceProvider {
...
class OP_AWeberAPIBase {
     ...
     * @access public
     * @return OP_AWeberEntry or OP_AWeberCollection
     */
    ...
    protected function readResponse($response, $url) {
        ...
        if (!empty($response['id'])) {
            return new OP_AWeberEntry($response, $url, $this->adapter);
        } else if (array_key_exists('entries', $response)) {
            return new OP_AWeberCollection($response, $url, $this->adapter);
        }
        return false;
    }
}
 ...
 * @version $id$
 */
class OP_AWeberAPI extends OP_AWeberAPIBase {
    ...
    public static function getDataFromAweberID($string) {
        list($consumerKey, $consumerSecret, $requestToken, $tokenSecret, $verifier) = OP_AWeberAPI::_parseAweberID($string);

        if (!$verifier) {
            return null;
        }
        $aweber = new OP_AweberAPI($consumerKey, $consumerSecret, null);
     ...
     * @return null
     */
    public function __construct($key, $secret, OptimizePress_Modules_Email_LoggerInterface $logger) {
        // Load key / secret
    ...
    public function setAdapter($adapter=null) {
        if (empty($adapter)) {
            $serviceProvider = new OP_AWeberServiceProvider();
            $adapter = new OP_OAuthApplication($serviceProvider);
     ...
     *      on the same page)
     * @return Object OP_AWeberCollection Object with the requested
     *     account data
     */
    public function getAccount($token=false, $secret=false) {
        if ($token && $secret) {
            $user = new OP_OAuthUser();
