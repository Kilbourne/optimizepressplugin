...
class OP_AWeberEntry extends OP_AWeberResponse {
    ...
    public function attrs() {
        ...
        if (!empty(OP_AWeberAPI::$_collectionMap[$this->type])) {
            foreach (OP_AWeberAPI::$_collectionMap[$this->type] as $child) {
     ...
     *
     * @access public
     * @param OP_AWeberEntry(List)   List to move Resource (this) too.
     * @return mixed OP_AWeberEntry(Resource) Resource created on List ($list)
    ...
    public function move($list, $last_followup_message_number_sent=NULL) {
        ...
        return new OP_AWeberEntry($resource_data, $url, $this->adapter);
    }
     ...
     * @access public
     * @throws OP_AWeberResourceNotImplemented
     * @return mixed
     */
    public function __get($value) {
        ...
        if (!empty($this->data) && array_key_exists($value, $this->data)) {
            if (is_array($this->data[$value])) {
                $array = new OP_AWeberEntryDataArray($this->data[$value], $value, $this);
        ...
        throw new OP_AWeberResourceNotImplemented($this, $value);
    }
    ...
    public function findSubscribers($search_data) {
        ...
        return new OP_AWeberCollection($data, $url, $this->adapter);
    }
    ...
    public function getParentEntry(){
        ...
        try {
            ...
            return new OP_AWeberEntry($data, $url, $this->adapter);
        } catch (Exception $e) {
            return NULL;
        }
    }
    ...
    protected function _parseNamedOperation($data) {
        ...
        foreach($data as $entryData) {
            $results[] = new OP_AWeberEntry($entryData, str_replace($this->adapter->app->getBaseUri(), '',
    ...
    protected function _methodFor($entryTypes) {
        ...
        throw new OP_AWeberMethodNotImplemented($this);
    }
    ...
    protected function _getCollection($value) {
        if (empty($this->_collections[$value])) {
            $url = "{$this->url}/{$value}";
            $data = $this->adapter->request('GET', $url);
            $this->_collections[$value] = new OP_AWeberCollection($data, $url, $this->adapter);
        }
    ...
    protected function _isChildCollection($value) {
        $this->_type();
        if (!empty(OP_AWeberAPI::$_collectionMap[$this->type]) &&
            in_array($value, OP_AWeberAPI::$_collectionMap[$this->type])) return true;
        return false;
    }
