...
class _WP_Editors {
    ...
    public static function enqueue_scripts() {
        ...
        wp_enqueue_script('word-count', false, array(OP_SCRIPT_BASE), OP_VERSION);

        if ( self::$has_tinymce ){
            wp_enqueue_script('editor', false, array(OP_SCRIPT_BASE), OP_VERSION);
        }

        if ( self::$has_quicktags )
            wp_enqueue_script('quicktags', false, array(OP_SCRIPT_BASE), OP_VERSION);

        if (substr($tinymce_version, 0, 1) < 4) {
            if ( in_array('wplink', self::$plugins, true) || in_array('link', self::$qt_buttons, true) ) {
                wp_enqueue_script('wplink', false, array(OP_SCRIPT_BASE), OP_VERSION);
                wp_enqueue_script('wpdialogs-popup', false, array(OP_SCRIPT_BASE), OP_VERSION);
        ...
        if ( in_array('wpfullscreen', self::$plugins, true) || in_array('fullscreen', self::$qt_buttons, true) )
            wp_enqueue_script('wp-fullscreen', false, array(OP_SCRIPT_BASE), OP_VERSION);

        if ( self::$has_medialib ) {
            add_thickbox();
            wp_enqueue_script('media-upload', false, array(OP_SCRIPT_BASE), OP_VERSION);
        }
    }
